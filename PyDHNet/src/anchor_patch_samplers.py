...
def sample_structure_anchor_patches(hparams, networkx_graph, meta_paths, node_type, max_sim_epochs): 
    ...
    all_patches = []
    print(f'Perform {n_samples} walks')
    i = 0
    while i < n_samples:
        node = np.random.choice(list(networkx_graph.nodes))
        ...
        if hparams['structure_patch_type'] == 'ego_graph': 
            ...
            subgraph = triangular_random_walk(hparams, networkx_graph, meta_paths, node_type, networkx_graph, hparams['sample_walk_len'], None,  None, True)
        else:
