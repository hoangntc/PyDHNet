...
class PyDHNet:
    ...
    def initialize(self):
        data_module = DyHNetDataModule(self.config)
        model_module = DyHNet(self.config)
    ...
    def generate_embedding(self, data_module, model_module, restore_model_dir, restore_model_name, output_dir, device='cuda:0'):
        args_inference = {
            'config': self.config,
